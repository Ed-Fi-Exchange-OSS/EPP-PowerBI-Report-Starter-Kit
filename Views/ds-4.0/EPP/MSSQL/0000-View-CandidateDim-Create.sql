...
CREATE OR ALTER VIEW analytics.epp_CandidateDim AS
...
SELECT Candidate.CandidateIdentifier AS CandidateKey
		...
		,CONVERT(VARCHAR,CandidateEducatorPreparationProgramAssociation.BeginDate, 112) AS BeginDateKey
		,CAST(CandidateEducatorPreparationProgramAssociation.EducationOrganizationId AS VARCHAR) as EducationOrganizationId
		,CAST(CandidateEducatorPreparationProgramAssociation.EducationOrganizationId AS VARCHAR) AS EducationOrganizationKey
        ...
        ,CONVERT(varchar,(COALESCE(CASE
			WHEN SUM(CASE
				WHEN Credential.CredentialIdentifier IS NOT NULL
					THEN 1
				ELSE 0
				END) > 0
			THEN CAST(MIN(Credential.IssuanceDate) as NVARCHAR) END, '')),112) as IssuanceDateKey
